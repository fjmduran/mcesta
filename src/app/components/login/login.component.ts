...
export class LoginComponent {
  ...
  private subsNewUser: Subscription; //declaro este objeto para poder anular la subscripción de añadir un nuevo usuario al salir de este componente

  public boolSpinner: boolean = false;
  public pwdIcon:string='visibility_off';
  public pwdType:string ='password';

  constructor(private auth: AuthService, private ngZone: NgZone, private router: Router, private _snackBar: MatSnackBar,private iconRegistry: MatIconRegistry, private sanitizer: DomSanitizer) {
  ...
  ngOnInit() {
    ...
    this.SiginForm.get('repite_pwd').setValidators([Validators.required, CustomValidators.equals(this.SiginForm.get('pwd'))]);
  ...
  public onLoginEmail(): void {
    ...
    this.auth.loginFirebaseEmail(email, pwd)
      .then((res) => {
        ...
        console.log(err);
        this.openSnackBar('Las credenciales no son válidas o el usuario ya está registrado con la cuenta de Google');
        this.boolSpinner=false        
      });
  ...
  checkUser(user: firebase.auth.UserCredential) {
    ...
    this.auth.checkUser(usuario).subscribe(data => {
      if (data.length == 0) {
  ...
  OnSigIn(): void {
    ...
    this.auth.siginFirebaseEmail(this.user.email, this.user.pwd)
      .then(res => {        
        this.subsNewUser = this.auth.checkUser(this.user).subscribe(data => {          
          if (data.length == 0) {
            this.auth.saveUser(this.user);            
            this.router.navigate(['/home']);
          } else {
            //si llego aquí es que se está intentando registrar un usuario que ya existe
            this.openSnackBar("Este email ya está registrado en la plataforma.")
            this.sigin = false;
          }
          this.boolSpinner=false
        },
      ...
      .catch(err => {
        ...
        switch (err.code){
          case "auth/email-already-in-use":{
            this.openSnackBar("El email ya está registrado, pulse en el botón de IDENTIFÍCATE")
            break
          }  
  ...
  ngOnDestroy() {
    if (this.subsNewUser != null) this.subsNewUser.unsubscribe();
  }

  ShowRegister(b: boolean): void {
    this.sigin = b;
    if (this.sigin) this.texto = 'Registro'; else this.texto = 'Accede';
  }
